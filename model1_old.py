...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 27 12:49:25 2025
...
def transition_depol_err(rho, depol_err, sys_qubit_ids, num_qubits):
    kraus_input = []      
    for key1, U1 in pauli.items():
        if len(sys_qubit_ids) == 2:
            for key2, U2 in pauli.items():
                mid_qubits = np.identity(int(2 ** int( np.abs(np.diff(sys_qubit_ids)) - 1)))
                if key1 != 'I' or key2 != 'I':
                    pauli_err = U1.tensor(operator(mid_qubits)).tensor(U2)
                else:
                    continue
        else:
            if key1 != 'I':
    ...
    kraus_input.append(np.sqrt(1-len(kraus_input)*depol_err).tolist() * np.identity(pauli_err.shape[0]))
    op = operator(np.identity(2 ** min(sys_qubit_ids))).tensor(kraus(kraus_input)).tensor(operator(np.identity(2 ** (num_qubits - max(sys_qubit_ids)-1))))
    return rho.transition(op)
...
class Model:
    ...
    def _run_instruction(self, sys, instruction, used_qubits, cross_talk_prob_params):
        ...
        sys.rho = transition_depol_err(sys.rho, depol_err, sys_qubit_ids, len(used_qubits))
        ...
        for cross_talk_qubits, p in zip( connections, probs):
            ...
            rho_i = sys.transition_qubit(ideal_gates['cz'], [used_qubits.index(q)  for q in cross_talk_qubits], in_place = False)
            rho_i = transition_depol_err(rho_i, depol_err, [used_qubits.index(q)  for q in cross_talk_qubits], len(used_qubits))
            rho_i -= sys.rho

#            rho_check = np.abs(rho_i.tolist())
#            print(rho_check.max(), rho_check.argmax())
    ...
    def _run(self, circuit, readout_qubits, used_qubits, params):
        ...
        # used_qubits = list(used_qubits) #cz gates are cool in that they don't do anything when one qubit is in zero state.
        ...
        for t in range(1):
            ...
            readout_probs = jnp.diag(sys.rho._data).real
            
            readout_probs = jnp.matmul(readout_probs, compute_prob_matrix(self.readout_err[used_qubits]))
        return readout_probs
#            output += np.exp(readout_probs)
