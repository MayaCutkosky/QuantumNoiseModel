...
#modified from function from chatgpt
def make_inverse_map(sys_qubits, max_id = None):
    if max_id is None:
...
def fixed_size_where(condition, num_vals):
    return jnp.argsort(condition)[-num_vals:]